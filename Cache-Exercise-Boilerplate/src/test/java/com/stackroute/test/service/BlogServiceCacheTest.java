...
public class BlogServiceCacheTest {
    ...
    @Test
    void givenBlogToSaveThenShouldReturnSavedBlog() throws JsonProcessingException {
        when(blogRepository.save(any())).thenReturn(blog1);
        Blog saveBlog = blogService.saveBlog(blog1);
       // assertEquals(blog1.getAuthorName(),saveBlog.getAuthorName() );
        ObjectMapper om=new ObjectMapper();
        String writeValueAsString = om.writeValueAsString(saveBlog);
        System.out.println(writeValueAsString);
        assertEquals(blog1,saveBlog );
    ...
    void givenBlogIdThenShouldReturnBlogWithThatId() {
        when(blogRepository.findById(blog1.getBlogId())).thenReturn(Optional.of(blog1));
        blogService.saveBlog(blog1);
        blogService.saveBlog(blog2);
